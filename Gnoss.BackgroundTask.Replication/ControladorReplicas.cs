...
namespace Es.Riam.Gnoss.Win.ServicioReplicacionVirtuoso
{
    internal class ControladorReplica : ControladorServicioGnoss
    {
        ...
        /// <summary>
        /// El n�mero de la conexi�n que se debe usar para actualizar virtusos.
        /// </summary>
        private string mCadenaConexion;

        /// <summary>
        /// Cadena de conexi�n que se debe usar para actualizar virtuoso.
        /// </summary>
        private VirtuosoConnectionData mVirtuosoConnectionData;

        /// <summary>
        /// Devuelve si la cadena de conexi�n para actualizar virtuoso es una BBDD Master.
        /// </summary>
        ...
        /// <param name="pTablaColaReplica">Nombre de la tabla de cola de esta r�plica</param>
                ...
                foreach (BaseComunidadDS.ColaReplicacionRow filaReplica in pFilasTransaccion)
                {
                    ...
                    try
                    {
                        if (!ProcesarFila(filaReplica, false, entityContext, entityContextBASE, utilidadesVirtuoso, loggingService, servicesUtilVirtuosoAndReplication))
                        {
                            ...
                            throw new Exception($"Error al replicar instrucci�n {filaReplica.OrdenEjecucion}. Query: {filaReplica.Consulta}. Transaccion: {filaReplica.InfoExtra}");
                        }
                    }
                    catch
                    {
        ...
        /// <summary>
        /// A partir del n�mero de conexi�n obtiene los dem�s datos necesarios para actualizar virtuoso.
        /// </summary>
        ...
        /// <summary>
        /// Procesa una fila de la cola de replicaci�n
        /// </summary>
        /// <param name="pFilaCola">Fila de la cola de replicaci�n</param>
        ...
        private bool ProcesarFila(BaseComunidadDS.ColaReplicacionRow pFilaCola, bool pReintentarSiFalla, EntityContext entityContext, EntityContextBASE entityContextBASE, UtilidadesVirtuoso utilidadesVirtuoso, LoggingService loggingService, IServicesUtilVirtuosoAndReplication servicesUtilVirtuosoAndReplication)
        {
            ...
            try
            {
                ...
                if (pFilaCola.Estado == 2)
                {
                    ...
                    //La replicaci�n hab�a fallado y ahora vuelve a funcionar, informo de ello. 
                    EnviarErrorYGuardarLog(new ExcepcionDeReplicacion("Mensaje informativo: La replicaci�n de virtuoso para la conexi�n '" + mFicheroConfiguracionBDOriginal + "' y la tabla '" + mTablaColaReplica + "' vuelve a funcionar CORRECTAMENTE. "), "Servicio de replicaci�n de virtuoso restituido", loggingService);
                }
            ...
            {
                hayError = true;
                string mensaje = "Excepci�n: " + ex.ToString() + "\n\n\tTraza: " + ex.StackTrace + "\n\nFila: " + pFilaCola["OrdenEjecucion"];
                ...
                if ((ex.InnerException != null) && (ex.InnerException.StackTrace.Contains("SQLSTATE: 37000") || ex.InnerException.Message.Contains("SQLSTATE: 37000") || ex.Message.Contains("Virtuoso 37000") || ex.InnerException.Message.Contains("syntax error") || ex.InnerException.StackTrace.Contains("syntax error")) || ex.InnerException.Message.Contains("Empty string is not a valid argument"))
                {
                    ...
                    {
                        EnviarErrorYGuardarLog(ex, "Servicio de replicaci�n de virtuoso restituido", loggingService);
                ...
                else
                {
                    ...
                    string mensajeError = "Error al replicar la transacci�n: " + pFilaCola.OrdenEjecucion + ". ";

                    //Compruebo si hab�a error previamente. Si es as�, no env�o emails continuamente. 
                    if (!excepcionLanzada)
                    {
                        excepcion = ex;
                        if (pFilaCola.Estado == 2)
                        {
                            mensajeError += "La replicaci�n de virtuoso para " + mTablaColaReplica + " est� PARADA. ";
                            // Creo la excepci�n, pero se lanza al final del foreach para actualizar el estado de la fila en BD
            ...
            if (hayError)
            {
                if (excepcion != null)
                {
                    //La replicaci�n ha fallado, lanzo la excepci�n
                    throw excepcion;
                }
            }
        ...
        /// <summary>
        /// Hacemos 3 intentos de la insercci�n en Virtuoso
        /// </summary>
        ...
        /// <summary>
        /// M�todo para enviar al servicio Modulo Base los par�metros enviados como par�metro extra
        /// </summary>
        /// <param name="pInfoExtra">Par�metros extra para que procese el servicio m�dulo Base.</param>
        ...
        {
            string[] delimiter = { "|;|%|;|" };
            //Insertamos en el base los nuevos par�metros
            foreach (string nuevaFila in pInfoExtra.Split(delimiter, StringSplitOptions.RemoveEmptyEntries))
            {
                try
                {
                    ...
                    //Tipo de acci�n (0 agregado) (1 eliminado)
                ...
                {
                    loggingService.GuardarLogError("Error al agregar al base. Conexi�n '" + mFicheroConfiguracionBDOriginal + "', tabla '" + mTablaColaReplica + "'" + Environment.NewLine + ex.Message + Environment.NewLine + ex.StackTrace, mLogger);
                }
            }
        }
        ...
        protected bool CargarDatos(string pNombreTabla, EntityContext entityContext, LoggingService loggingService, IServicesUtilVirtuosoAndReplication servicesUtilVirtuosoAndReplication)
        {
            ...
            // La primera vez que arranca el servicio, se trae las filas que hab�an fallado antes. 
            short estadoMax = 2;
